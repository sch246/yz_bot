...
模块可以在顶层写 `OP_ONLY = True`，声明"只在 op 发起的轮里可见、可加载"（目前只有 `op.py` 这么做）。这是**按轮**的门控，不是按进程：目录来自进程全局的 registry，而"这一轮谁在说话"每次现算，所以同一个群这轮由管理员触发就能看到它、下轮由普通成员触发就看不到。三层都拦：渲染目录时过滤（`tools.op_tool_visible`）、`SessionBinding.load` 拒绝（模型记得名字直接 load 也没用）、工具执行时自己再查一次 `op.is_op`。**不在 registry 里删模块**——那样会把能力从所有窗口一起拿走；理由见 [op 工具集提案](working/proposals/op-toolbox.md) 的决定三。
...
- **装不回来的名字就地丢掉。** 判据沿用 `visible`：`OP_ONLY` 模块只在 op 那一轮可见，窗口在普通轮里恢复它必然失败，留着只会每轮报一次加载失败；丢掉无害——上一轮它已经用完了。丢掉的同时回写 storage，名单不会一直挂着装不回来的名字。
