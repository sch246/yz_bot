...
图片读取是当前窗口开关。开启时，声明支持 vision 的模型接收转换后的图片；不支持 vision 的模型先调用视觉模型生成文字描述，并把 URL 到描述的映射保存在 `llm_system/description_cache`。关闭时，历史消息中的图片 part 会降级为 `[图片 URL: ...]` 文本：既避免把 `image_url` 发给纯文本模型，又让模型可以按需调用 `recognize_image`。如果开启读取但没有可用的视觉模型，也会保留 URL 并降级为文字说明。
...
描述缓存以完整图片 URL 为键，值包含 `description` 和 `cached_at` ISO 日期。只有视觉调用成功才写入；命中后直接复用文字而不再请求视觉模型。新写入使缓存超过 500 条时，会删除缓存日期距当天超过 15 天的条目；近期条目即使超过 500 条也保留。历史字符串值仍可命中，在首次命中或清理时原地升级并以当天作为初始日期，避免上线时突然失效全部旧缓存。
