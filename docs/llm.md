...
主窗口 mail reader 在每次模型子请求读完 mail 后，复用本次请求的已编号事件文本 token 估算：超过 `max_token` 的 75% 时，在**模型可见的末尾 hint** 中只显示“上下文占用 N%”（比例向上取整）；否则不显示。它不计事件条数、不替模型选择要总结的内容，也不是聊天结束后向 QQ 发消息的 `#hint` 命令。这个比例只相对于历史文本预算，不是完整 API 请求的实际 token 占比；未读 mail 可能一次越过阈值，提醒不保证提前一轮出现，更不自动执行总结。
