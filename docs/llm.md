...
### 工具变动改为追加式上下文（设计方向已定，尚未实现）

现状里模块目录是一条 system 消息，由 `SessionBinding._render` 就地改写，而 `chat.init_chat` 把它排在全部历史之前。这有两个后果：上下文里最常变的一段坐在最前面，就地改写会让它之后的全部前缀缓存失效；模型也只有主动调 `list_tools` 才知道磁盘变了。

方向是让工具变动作为一种**上下文条目追加**到末尾，前面的部分一个字都不动。可参考 deepseek-harness 的 `packages/context/*`：那里的上下文插件从不修改已有消息，而是往历史里追加带来源的 user 消息，首次注入完整基线、之后只追加变更列表，内容包在插件自己拥有的 `<system-reminder>` 框架里，且工作区文本中字面的结束标记会被转义，使被引用的内容无法关闭框架。因为注入的是普通历史消息，它与其它对话内容一样可回放、可压缩、可从会话日志重建。

这是 llm 侧的通用能力，不属于 `mods/tools`；落地时要连带改 `Chat` 的消息组装。

它与"末尾 hint"是两件事，不要合并：hint 是更轻的一层，用于被动提醒，始终追加在上下文末尾而不进历史，不承载模块目录这类重内容。
...
还缺插话与打断。入站并没有被生成挡住：`link.dispatch` 在独立线程上，所以生成期间消息照样到达——现状是第二条 at 会**再起一轮并发的 `chat()`**，两轮各自读上下文、各自发言，而不是并入同一轮。`^C` 对生成也无效：`bot._route` 的 `^C` 分支能跑，但 `context.cancel` 只唤醒登记过的 waiter，而工具循环没有登记。

设计方向已定，尚未实现：

- 两者都按**窗口**登记而不是 `(窗口, 用户)`，因此任何人都能插话、任何人都能 `^C` 打断。理由是 LLM 上下文本就按窗口共享，只让触发者能停会让群里其他人无法制止一轮跑偏的生成；这个粒度与 `context.interaction_key` 不同，需要单独的窗口级登记。
- 插话走队列：中途到达的消息进队尾，在本轮返回后、或下一次调用模型前（例如工具返回后自动续问的那一步）一并带入，而不是打断当前请求。
- 分级：只有原本就会触发聊天的消息（at、link 命中）才**触发**一次新的插话调用，普通消息只**进队列**不触发。这样既让模型看到更多上下文，又避免普通闲聊无限续聊。
- `^C` 打断需要工具循环在每轮之间检查窗口级取消标记（`InteractionCancelled` 已存在），理想情况下还能中断流式读取。
