...
- 装配由 `chat.build_context` 完成，聊天历史与工具记录**共用一份 token 预算**。各算各的等于没算——两边都在限额内，加起来照样超。所以归并发生在预算**之内**：按时间从新到旧走一条合并后的时间轴，谁先够到边界谁被丢。
- 归并的单位是**一轮**：`assistant(tool_calls)` 和它的 `tool` 结果之间插进聊天消息会拆散配对，所以每轮带一个时间、整体落位，预算也按整轮扣。同一时刻聊天排在工具轮之前，因为工具是被那条消息触发的。
- **早于最老那条聊天消息的工具轮不进上下文。** 没有对应聊天做锚点的操作，模型无从判断它当时在回应什么，孤零零摆在最前面只会误导。
- 归并出的顺序**不保证与当时完全一致**：一轮里几个并发调用完成时间不同，工具执行期间到达的消息其真实先后也无法从一个时间点还原。这是明知的近似——拿回"发生过什么、大致在什么时候"，不追求逐条复原。
- 没载入不等于丢失。存储侧**唯一**的上限是保留期（`MAX_AGE_SECONDS`，7 天），没有条数上限也没有字数上限：一次巨量输出照存不误。进不进上下文由预算决定，还能不能取回由保留期决定，这是两件事。
...
- 过期同样按整轮清理，半轮会重建出配不上的 `tool_calls`。
...
- **收缩是可逆的**：标记而非删除，原文按保留期继续留着，`recall_ops(cids)` 随时能取回（`#ops` 里也照常显示，标着"已收缩"）。唯一真正的删除是过期。这条不是顺手加的宽容：删除会让收缩变成不可逆的销毁，模型每次收缩都得先赌自己的结论没写错；可逆之后收缩才是个能放心做的动作。
