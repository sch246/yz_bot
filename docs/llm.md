...
中心 reader 从 `AGENT_WINDOW` 的统一已读信息流选取最新可见后缀，最多 `max_events` 条且不超过 `max_token`；输入、完整输出、结果和摘要共同计数，被覆盖原文不占名额。默认 500 条／40000 token；op 可用 `#limit` 或 `#agent limit` 修改同一份全局设置。旧窗口覆盖保留原数据，只供独立 `.chat` 读取。`聊天开始／结束`是普通内容。`get_msgs()` 保留为窗口级辅助查询，不是中心开局的读取入口。

中心 agent 可用 `cover_events(ids, conclusion)` 将已读正式号归入本次行动的结论，节点沿用输出号和行动位置。显式调用如 `cover_events(["20260923-4", "20260923-5"], "结论")`；范围调用如 `cover_events(ids=[], conclusion="结论", anchor="20260923-4", before=2, after=3, kinds="input")`，或改用 `start`／`end`。`ids=[]` 是当前工具 schema 为保留旧位置参数而要求的范围写法。成员可跨来源窗口，也可重复被不同节点覆盖；首次覆盖仍须在当前已读可见流中，旧号经明确 `recall_events` 且整段结果进入模型后也可点名。输出与返回、已确认的 `say`／回声保持强绑定：筛选只选种子，实际闭包可越过范围、种类和来源，甚至超过 40 条；任一最终成员不可覆盖则整次失败，不自动补入未读成员或静默删成员。覆盖只改变默认投影，不删原文或原号；`recall_events` 可按原号反查实际冻结成员。独立 `.chat` 和子代理不能提交中心覆盖。
...
`oplog` 从已登记输入的模型可见文本、输出正文与行动参数、返回参数与正文中识别当时已存在的正式号；中心输出允许指向旧窗口号，私有记录仍受窗口限制。`event_links(ids)` 或 `event_links(anchor="20260923-4", before=2, after=2, kinds="result")` 可查看一跳文本引用、被引用者、覆盖成员／所属节点和结果来源。筛选只限制查询根，邻接边完整返回；它不读取正文，也不授予覆盖信用。覆盖、文本出现编号与某结论的真实依据是不同关系；搜索树可沿节点 `Y` 的覆盖成员找到返回 `R`，再沿 `R` 的文本引用找到旧记录 `E`。这里尚不实现自动退休、高度计算或语义证据核验；私有窗口的邻居可见性暂不随选择器改变。

`event_span(anchor, before, after)`、`recall_events`、`event_links`、`cover_events` 共用正式事件选择：显式 `ids`、`anchor` 加前后数量，或同时给 `start`／`end`，三者只能选一种。范围沿 `oplog._events` 的唯一已读追加顺序含端点解析出实际 IDs，最多 40 条种子（筛选前）；`kinds` 和 `source` 随后才过滤，不重排，也不按筛选命中补足。缺选择器、混用、不完整区间、非法种类或来源都会报错。`event_span` 仍只列编号，不读取正文；`recall_events(start="20260923-4", end="20260923-8")` 则以解析后的 IDs 读取原文，含已覆盖、隐藏或收缩的可读事件。范围反查返回冻结的 `resolved_ids`；长结果续页必须改用这组显式 `ids` 和返回的 `offset`，避免后续输出或结果追加改变范围。`pull` 消耗未读 FIFO，并给世界事件首次分配 input 号；反查则由当前输出 `O` 发起行动，旧内容随新的工具 result 事件 `R` 入流，`R` 的文本引用旧事件 `E`（`O → R → E`）。旧 `E` 不移动、不复制、不重新编号，也不消耗未读 FIFO。通常总结这次探索时覆盖 recall 的 `O/R`，结论再引用旧号，形成“结论 → 回看结果 → 旧事件”；完整回看后也保留直接覆盖旧 `E` 的兼容能力，但那是另一次明确选择，并非默认等价。覆盖可见性信用只在整个工具结果实际进入模型且读完后按解析出的编号授予，不在工具执行时提前授予。选择器不扩展到 `peek/fetch/status/pull`、`recall_ops/condense_ops` 或 `say_links`。

一次模型输出的全部同步工具返回完成时立即登记一个普通 result R；正常下一子请求先追加当前会话暂存的完整 R 投影，再追加通知和显式 pull，不按同批工具数量均分、二次截断或自动分页。单独调用 `peek` 或 `recall_events` 时，其整条返回就在这个 R 中。同一输出里的工具彼此看不到结果。若 `say(final_call=true)` 结束、请求取消或失败，不为结果强迫续轮，也不做交付确认；下次激活按普通历史预算重建，可能看不到全文，需要时用 `recall_events` 主动反查。极窄取消窗口可能让模型没读到已经登记的 R。返回正文展示的旧聊天档案行不分别取得 input 号、不消费 FIFO，且不能充当或确认 `say` 的 QQ 回声；要给未读来源消息分配 input 号须 `pull`，已有 input 号的旧消息则按该号 `recall_events`。
...
`init_chat()` 依次装配主设置选定的提示词、base、工具说明、中心 agent 身份说明与全局已读后缀；`_activate_chat()` 从全局 `agent.active_tools` 恢复主工具状态。独立 `.chat` 仍按发起窗口装配。同步 `peek` 返回直接跟在本次输出后；正式拉取只在下一安全子请求边界追加新输入，通知不含未读正文。
...
- **事件数是宽护栏，token 是主预算**：默认 `max_events = 500`、`max_token = 40000`。这样密集工具调用不会仅因跨过 20 条就让刚说过的话消失，同时总文本仍受 token 上限约束。旧窗口覆盖不自动迁入全局主体。
...
**旧档案主动翻阅。** 中心 agent 用 `peek(source, before)` 按本地 `_log_origin` 锚点预览有界片段，不减少当前 FIFO 未读。远端旧档先用 `fetch(g/u)` 固化成有名字的信源；完成后仍以同一个 `status/peek/pull` 接口查看与读取，不让模型管理 NapCat 请求页。预览返回在同步工具批次完成时随整批取得一个 R 号。默认上下文不再为补满预算自动倒扫 chatlog。`#hint` 不触发旧档阅读。

### 操作历史与结论收缩

模型子请求的一次完整输出（思考、正文、零到多个行动）在派发工具之前取得一个全局 `YYYYMMDD-N` 号；行动以 `号#位置` 指名。QQ 输入和通知在阅读时、输出在派发工具前、同步结果在批次完成时进入同一追加式信息流；未读到达不预占正式号。来源窗口各自持有持久 pending；中心 reader 选择窗口后按 FIFO 有界正式读取。到达与正式登记事实写入 `data/event_stream/YYYYMMDD.jsonl`，chatlog 仍是 QQ 消息正文权威。

- 中心上下文沿全局 `max_events`、`max_token` 选已读事件，编号输入、输出、结果按阅读顺序投影；开局只给不含正文的通知，`peek` 不消费未读，`pull(source, count)` 从选定 FIFO 队首有界读取。连续聊天执行期间临时保留原生工具配对，下一子请求前撤掉，换成统一投影；`.chat` 与子代理仍用各自原生配对，原生工具 ID 只供供应商配对，不是稳定记忆号。
- `say` 返回和 QQ 自发回声以完全相同的 `message_id` 且双方均唯一为确认关联条件；早到回声仅保留事实，不靠文本或时间猜。超时、缺少 `message_id`、重复回声都不宣布确认。
- 思考只留“曾有思考”标记，不持久化思考全文；自言自语正文与行动共享一个输出号，并跨轮按统一纯文本投影重建，直到被覆盖或超出上下文预算。一次同步工具批次完成时登记一个普通结果号；当前会话的下一子请求完整投影，后续激活则按普通历史预算有界截取，提示用 `recall_events(offset)` 续读。覆盖只改默认可见性，不删原文。私有 `.chat` 仍可用 `condense_ops` 收缩自己已经收到的原生工具配对；`recall_ops` 按行动引用取回。清理可见性不销毁日志、不复用号码，长期遗忘和物理退休另待裁定。

- 流式结果中途截断不会登记半个输出或派发行动；输出写入日志失败则不派发。行动开始后不会自动重放：恢复仅恢复未读 mail，而非重跑已登记的动作。

统一正式号表示中心信息流的登记顺序，不表示各 QQ 窗口的消息发生顺序；极窄的取消窗口里，已登记的同步 R 可能尚未送进模型。独立 `.chat` 和子代理可先在私有请求中读到原生工具返回，不替中心推进 mail 水位；旧 per-window 正式号保留原身份，中心可按原号反查。
...
Chat.chat
  ...
  → 工具批次完成即登记一个 R，下一请求先完整投影，再读取通知和显式 pull
  → 直到某轮不再调用工具
```
...
完整子响应先登记一个输出号、再派发工具；连续聊天在执行中临时暂存 assistant/tool 原生配对，执行完即以编号输出投影替换，并登记一个整批结果 R。下一次请求先完整投影当前会话的 R，再追加通知和显式 FIFO pull；跨轮重建沿用普通历史预算、身份、顺序与自言自语正文。流式文本仍由回调逐段产出用于终端显示，不会分拆成多个输出事件；工具失败不会回滚已经发生的行动。单句 `.chat` 不读取 mail，继续通过原生配对获取同步返回。
...
中心 reader 登记在 `context.WindowTurn` 的 `AGENT_WINDOW` 键下；各来源窗口的 `Mailbox` 跨轮保存 FIFO 未读。通知递交、未读正文与 pull 所用的固定上沿分别持久记录，不以一个红点代替。相关窗口的群友可用 `^C` 取消共享请求，未读与其它窗口的通知仍留在信息流里。

- **先接纳，后阅读。** 路由在同一窗口锁内写 chatlog/history 与 arrival；Bot 的 `message_sent` 回声也入列，但不再执行命令。中心开局只递交不含正文的通知，既不偷读末段，也不把未读整段追加。
- **预览与正式拉取分开。** `peek` 读有界最近、旧档或独立信源队首，整批同步工具返回形成一个 R，正常续轮直接交付，不动未读。`pull(source, count)` 在安全边界从指定 FIFO 队首正式取模型要求的数量；单条超大内容可按档案位置续读。
- **召唤不是正文。** 模型输出持久后确认已递交通知；同一条召唤不因正文仍 pending 而反复唤醒。只有后来新到的召唤或模型已经显式安排的 pull 会让中心 reader 继续；`final_call` 或取消后的 R 留在普通历史，下次激活按预算重建，普通未读本身也不单独开轮。
- **`#` 只参与控制，不进入模型正文。** 这类事件可留下到达和激活事实（`#poke` 会唤醒），但不会作为聊天正文投影；本地子命令仍就地执行。
- **收尾没有缝。** `context.finish_turn` 在全局 reader 登记锁内检查通知与显式 pull；新召唤要么被当前 reader 接住，要么在注销后领取新 reader。已完成而本轮不能继续的 R 保存在普通历史，下次激活按预算重建，需要全文可反查。
