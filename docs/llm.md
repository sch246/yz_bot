...
主窗口 mail reader 还会在每次模型子请求读完 mail 后，按本次 `Chat.messages` 中实际可见的已编号事件重算一次**模型可见的压力 hint**；它不是用户 `#hint` 命令，也不进持久流、聊天历史或 QQ。事件数或文本 token 估算达到对应 `#limit` 的约 75% 时出现，低于两项阈值时消失。它尝试点名最老、当前可覆盖且有明显净收益的一段（最多 12 条），列出连带事件的正式号、条数和估算 token；候选与真实 `cover_events` 共用只读资格和闭包检查。若最老段关联未读、隐藏、已收缩、未确认回声等不可覆盖成员，或不足以抵消一次摘要输出与返回的预算，hint 只报告压力和阻碍，不伪称已压缩。75%、12 条和摘要预留 120 token 是暂定启发式，不是安全保证：限额只管历史选取，不等于 API 完整上下文容量；未读 mail 可能一次越线，首次请求可能已过大，也不保证硬上限前至少提醒一轮。模型仍决定是否写摘要，覆盖本身不删除原文。
