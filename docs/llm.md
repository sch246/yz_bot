...
还缺插话与打断，维护者已确认是问题。入站并没有被生成挡住：`link.dispatch` 在独立线程上，所以生成期间消息照样到达——现状是第二条 at 会**再起一轮并发的 `chat()`**，两轮各自读上下文、各自发言，而不是并入同一轮。`^C` 对生成也无效：`bot._route` 的 `^C` 分支能跑，但 `context.cancel` 只唤醒登记过的 waiter，而工具循环没有登记。两者共用一个未决问题：`context.interaction_key` 是 `(窗口, 用户)`，而 LLM 上下文按窗口共享，所以要先决定谁的 `^C` 能停掉群里这一轮、别人的插话又该并入哪一轮。
