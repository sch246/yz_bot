...
- 装配由 `chat.build_context` 完成，**截断规则只有一套**——聊天消息那套（`max_msg` + `max_token`）。操作记录不自己算预算、也没有自己的保留期，它**依附于聊天窗口**：留下来的最老那条消息之后发生的工具轮才进上下文，更早的连同聊天一起滚出去（`oplog.forget_before` 顺手回收）。曾经这两侧各有一套规则，拆了：它们描述的是同一件事"多早以前的事情还算数"，答案有两个就意味着两处调参、两处解释，总有一处会先漂。
