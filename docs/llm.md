...
`get_msgs()` 从当前窗口的统一已读信息流向前选取，最多取 `max_events` 条可见事件（输入、完整输出、结果及摘要共同计数），累计不超过 `max_token`；被覆盖的原文不占条数，遇到“聊天开始／结束”边界仍停止。源码默认分别为 20 和 50000，窗口配置可以覆盖；旧窗口的 `max_msg` 仅在尚无 `max_events` 时作为读取兼容值。没有聊天消息也可以载入摘要及其他可见事件。

主窗口模型可用 `cover_events(ids, conclusion)` 将已读的正式事件号归入当前行动的结论；覆盖节点使用本次输出号加行动位置，不另发编号。一次提交覆盖输出的完整行动批次、结果和唯一确认的 `say` 回声；未确认的自发回声不能先单独覆盖，跨窗口事件不能覆盖。成功后当前会话及下一轮自动投影不再载入成员，`recall_events` 仍能按原号反查，包括总结节点实际冻结的完整成员集合；反查不解除覆盖。覆盖只动事件流的投影与持久索引，不改原始聊天历史或磁盘原文；再次用 `cover_events` 可盖住上一层可见总结，而旧层覆盖不会因此失效。`.chat` 单句及子代理只能保留自己的原生工具收缩，不能提交主窗口覆盖；主窗口正式号的顺序只描述其 mail reader 的读取，不表示所有私有模型首次看到结果的时序。旧 chatlog 中能稳定定位、带 `message_id` 且未撤回的消息与戳一戳，在主模型实际读取时按档案位置补入正式号；v0 或缺消息号的旧消息保守跳过。**迁移限制：**若此前已有缺少档案来源定位的正式输入日志，同一条旧消息可能再次补号；部署前须核实或制定迁移策略，不把这层称为已完成全量兼容。

覆盖成员现在允许多重归属：已被某节点覆盖的正式事件可再次点名，和当前可见的新成员组成另一份总结；此前未覆盖的成员仍必须在这次主窗口的可见已读流中。各节点分别保存实际冻结成员，`_covered` 仅是默认隐藏的并集；重复引用不解除隐藏，也不放宽同窗口、输出／返回／已确认回声整批绑定和未完成行动检查。已隐藏但未被覆盖的记录仍不能凭猜测首次覆盖。

`oplog` 另从已登记输入的模型可见文本、输出正文及行动参数、返回参数及正文中识别正式号；只接受该记录登记前已存在的同窗口目标，行动位置也必须有效。它按记录重建正向引用和反向索引，`event_links(ids)` 可查看直接文本引用、被引用者、实际覆盖成员／所属节点和结果的来源输出，`recall_events` 也附上该记录识别到的直接文本引用。覆盖、文本出现编号与某结论实际依据什么是不同关系；搜索树时可沿 `Y` 的覆盖成员找到返回 `R`，再沿 `R` 的文本引用找到旧记录 `E`，不只读取 `cover_events.ids`。这只提供可逐层查询的引用入口，不实现自动退休、高度计算或语义证据核验。

主窗口 mail reader 在每次模型子请求读完 mail 后，复用本次请求的已编号事件文本 token 估算：超过本窗口 `pressure_percent` 阈值（默认 75%，由 `#limit` 的可选第三项设置）时，在**模型可见的末尾 hint** 中只显示“上下文占用 N%”（比例向上取整）；否则不显示。它不计事件条数、不替模型选择要总结的内容，也不是聊天结束后向 QQ 发消息的 `#hint` 命令。这个比例只相对于 `max_token` 历史文本预算，不是完整 API 请求的实际 token 占比；未读 mail 可能一次越过阈值，提醒不保证提前一轮出现，更不自动执行总结。

Bot 也可调用 `edit_hint(text)` 整体替换当前群聊或私聊窗口的待办，传空字符串清空。文本以独立的 `agent_hint` 键持久保存在该窗口的 storage；每次模型子请求从该键重新生成末尾 hint，因此修改在下一子请求或下一轮可见，hint 自身不逐版追加到历史（工具调用仍照常留在信息流），也不会发送 QQ 消息。它与上述压力提示并列，和向 QQ 发结束状态的 `#hint` 配置完全独立；跨窗口不共享。
...
本窗口待办使用 `edit_hint` 保存为可重算的末尾 hint，而不是把每个旧版 hint 再挂到模型末尾；做完后由模型整体更新或清空。长期可复用的经验则可写为 `mods/tools/*.md` Skill，经 `reload_tools` 应用、`load_tools` 按需插入上下文。Skill 不代替聊天原文索引：需要核对来源的结论应保留正式事件号，原话仍由 `recall_events` 反查。
...
`image`（图片档位）、`reasoning`（工具循环内是否带回思考内容）、`tools`（工具状态的呈现方式）、`max_events`、`max_token`、`pressure_percent` 是同一类东西：**一份窗口级的值 + 一个缺省值**。它们共用一张表（`mods/chat.WINDOW_SETTINGS`），每行是「storage 键 / 默认值 / 归一化」，读取一律走 `chat.window_setting(name)`——窗口里写过的合法值优先，否则默认值。加一项配置就是加一行；归一化负责把存坏的值拉回默认，所以读取端永远拿得到能用的值。
...
**旧档案按需补入。** 主模型阅读前，只有当前事件与 token 预算尚有余量时才从 chatlog 倒序寻找可稳定定位的旧记录；补入时按档案位置去重并分配正式号，查询与 `#hint` 不触发这一步。旧窗口的 `max_msg` 值只作 `max_events` 的读取回退；新写入使用 `max_events`。

### 操作历史与结论收缩

模型子请求的一次完整输出（思考、正文、零到多个行动）在派发工具之前取得一个全局 `YYYYMMDD-N` 号；同一输出的行动用 `号#位置` 引用，不再为每次工具调用单独写 `opN`。`oplog` 现在是跨窗口共用的追加式信息流：未读的 QQ 事件与工具结果只排进 mail、不预占正式号；模型在下一次子请求前读取时按 FIFO 分配正式号。到达和已读取事实写在 `data/event_stream/YYYYMMDD.jsonl`，可跨重启恢复未读，`chatlog` 仍是 QQ 消息自身的权威，不在本次迁移旧 `oplog` 文件。

- 上下文装配沿当前窗口的 `max_events`、`max_token` 和「聊天开始／结束」边界选统一已读事件；已编号的输入、输出、结果按读取顺序投影为普通 `user` 历史文本。连续聊天执行期间临时保留原生工具配对，下一子请求前撤掉，改由统一投影按输出 A、待读邮件 B、结果 C 顺序呈现，结果正文不会重复发送。`.chat <正文>` 是不读取窗口 mail 的单句请求，子代理有独立私有上下文；这两条路仍用原生配对得到自己的同步返回，并在原生输出附上统一行动引用供收缩使用。原生工具 ID 仅供供应商配对，不是第二套稳定身份；结果待读取时才赋正式号。
- `say` 返回和 QQ 自发回声以完全相同的 `message_id` 且双方均唯一为确认关联条件；早到回声仅保留事实，不靠文本或时间猜。超时、缺少 `message_id`、重复回声都不宣布确认。
- 思考只留下「曾有思考」标记，不持久化思考全文；自言自语正文同轮可见，跨轮投影省略正文，无行动输出完全不载入历史，但持久输出事件仍留原文供 `recall_events` 反查。输出正文与行动描述共享一个号。结果原文不会自动截断，`condense_ops` 按整次输出的行动引用移除**当前与后续**投影中的输出大参数和结果；私有原生上下文可在窗口结果尚未读到时收缩自身已见的原生配对，结果正式编号后继承收缩状态。`recall_ops` 可按输出号及位置反查已编号结果，私有模型还能反查自身已读、窗口尚未赋号的返回；其他模型不能凭猜测引用提前读取未读邮件。`recall_events` 可按正式号反查可见输入、输出或整批返回，不能反查原本被 `#` 过滤的消息。清理可见性不销毁日志也不复用号码。大结果的持久载荷仍会增长，摘要和删除策略另待裁定。

- 流式结果中途截断不会登记半个输出或派发行动；输出写入日志失败则不派发。行动开始后不会自动重放：恢复仅恢复未读 mail，而非重跑已登记的动作。

窗口信息流编号描述主窗口 mail 的读取顺序；独立 `.chat` 或子代理能先在自己的私有请求中读到原生工具返回，主窗口随后再按 FIFO 给 B、C 编号。因此不能把主窗口编号宣称为所有模型的首次认知顺序。私有反查只放行该会话已接收的自身结果，不替主窗口推进水位线。
...
- **「已用上下文」的出口是 `chat.context_usage()`**，hint 只调它、不自己算。它是**本地估算**：优先读取最近一次模型子请求实际收到的已编号事件文本快照，包含那次请求读完的整段 mail，而不是结束时重新按限额选历史；不含系统提示和工具 schema，也不是供应商返回的实际 token 数。
...
每次顶层 `_activate_chat()` 增加一次调用计数并恢复一次窗口工具；纯 `init_chat()` 只装配，不产生这两项生命周期副作用。子会话不经过顶层激活，因此不会增加同一种次数，但响应 usage 仍会累加 token 成本。费用按供应商返回的 usage 分三段算——输入未命中、输入命中缓存、输出——单价取自模型元数据，再按发起请求的时刻落在高峰还是空闲取档（见 `mods/llm/pricing.py`）；usage 可以在跨过峰谷边界后才到达，档位仍取请求发起时。命中缓存那部分是**必须**分开算的：聊天的 prompt 绝大多数是重复上下文，一律按未命中价算会把费用高估一个数量级。命中数由供应商上报（DeepSeek 的 `prompt_cache_hit_tokens`，OpenAI 一系的 `prompt_tokens_details.cached_tokens`），两边都报不出来时整段按未命中价算——宁可高估，也不静默少算。`.chattop` 按自然月（`YYYY-MM`）独立记账，其“调用次数”不能简单理解成实际向供应商发出的全部 HTTP 请求数，工具循环和子任务都可能令一次顶层聊天产生多次请求。
