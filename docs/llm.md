...
中心 agent 可用 `cover_events(ids, conclusion)` 将已读正式号归入本次行动的结论，节点沿用输出号和行动位置。显式调用如 `cover_events(["20260923-4", "20260923-5"], "结论")`；范围调用如 `cover_events(ids=[], conclusion="结论", anchor="20260923-4", before=2, after=3, kinds="input")`，或改用 `start`／`end`。`ids=[]` 是当前工具 schema 为保留旧位置参数而要求的范围写法。覆盖范围没有条数上限；成员可跨来源窗口，也可重复被不同节点覆盖。首次覆盖仍须在当前已读可见流中，旧号经明确 `recall_events` 且整段结果进入模型后也可点名。输出与返回、已确认的 `say`／回声保持强绑定：筛选只选种子，实际闭包可越过范围、种类和来源；任一最终成员不可覆盖则整次失败，不自动补入未读成员或静默删成员。覆盖只改变默认投影，不删原文或原号；`recall_events` 可按原号反查实际冻结成员。独立 `.chat` 和子代理不能提交中心覆盖。
...
`recall_events`、`event_links`、`cover_events` 共用正式事件选择：显式 `ids`、`anchor` 加前后数量，或同时给 `start`／`end`，三者只能选一种。范围沿 `oplog._events` 的唯一已读追加顺序含端点解析出实际 IDs，不设条数上限；`kinds` 和 `source` 随后才过滤，不重排，也不按筛选命中补足。缺选择器、混用、不完整区间、非法种类或来源都会报错。`recall_events(start="20260923-4", end="20260923-8")` 直接返回解析后的完整事件及冻结的 `resolved_ids`，含已覆盖或旧版已收缩的可读事件；它不进入 FIFO、不按字符分页，也不拆成多条模型消息。`pull` 消耗未读 FIFO，并给世界事件首次分配 input 号；反查则由当前输出 `O` 发起行动，旧内容随新的工具 result 事件 `R` 入流，`R` 的文本引用旧事件 `E`（`O → R → E`）。旧 `E` 不移动、不复制、不重新编号，也不消耗未读 FIFO。通常总结这次探索时覆盖 recall 的 `O/R`，结论再引用旧号，形成“结论 → 回看结果 → 旧事件”；完整回看后也允许直接覆盖旧 `E`，但那是另一次明确选择。覆盖可见性信用只在整个工具结果实际进入模型后按返回的 `resolved_ids` 授予，不在工具执行时提前授予。选择器不扩展到聊天档案、未读 FIFO 或 `say_links`。

一次模型输出的全部同步工具返回完成时立即登记一个普通 result R；正常下一子请求先追加当前会话暂存的完整 R 投影，再追加通知和显式 pull，不按同批工具数量均分、二次截断或自动分页。单独调用 `mentions`、`read_messages` 或 `recall_events` 时，其整条返回就在这个 R 中。同一输出里的工具彼此看不到结果。若 `say(final_call=true)` 结束、请求取消或失败，不为结果强迫续轮，也不做交付确认；下次激活按普通历史预算重建，可能看不到全文，需要时用 `recall_events` 主动反查。极窄取消窗口可能让模型没读到已经登记的 R。返回正文展示的旧聊天档案行不分别取得 input 号、不消费 FIFO，且不能充当或确认 `say` 的 QQ 回声；要给未读来源消息分配 input 号须 `pull`，已有 input 号的旧消息则按该号 `recall_events`。`mark_read` 只推进当前 FIFO 水位，不产生 input；之后仍可用 `read_messages` 查 chatlog。
...
`init_chat()` 依次装配主设置选定的提示词、base、工具说明、中心 agent 身份说明与全局已读后缀；`_activate_chat()` 从全局 `agent.active_tools` 恢复主工具状态。独立 `.chat` 仍按发起窗口装配。同步 `mentions`、`read_messages` 和 `recall_events` 直接跟在本次输出后；正式拉取只在下一安全子请求边界追加新输入，通知不含未读正文。
...
**旧档案主动翻阅。** 中心 agent 用 `read_messages(window, message_id|origin, before, after)` 在指定聊天窗口查本地档案，不减少当前 FIFO 未读；QQ `message_id` 命中多条时须加 `timestamp` 或用稳定 `origin` 消歧义。远端旧档先用 `fetch(g/u)` 固化成有名字的 FIFO，模型不管理 NapCat 请求页。`mentions` 与档案查询的返回在同步工具批次完成时随整批取得一个 R 号，不给其中每条聊天记录分配 input 号。默认上下文不再为补满预算自动倒扫 chatlog。`#hint` 不触发旧档阅读。
...
- 中心上下文沿全局 `max_events`、`max_token` 选已读事件，编号输入、输出、结果按阅读顺序投影；开局只给不含正文的通知，`pull(source, count)` 从选定 FIFO 队首有界读取，`mark_read(source)` 只推进调用时已有的未读水位。DeepSeek 主模型的完整 O/R 批次保留原生 assistant 思考、正文、工具调用与 tool 返回，重启后从同一事件流重建；旧记录、换模型、行动不完整或配对字段不符时，整批用明确的已编号文本投影。原生工具 ID 只供供应商配对，不是稳定记忆号。`.chat` 与子代理仍用各自原生配对。
- `say` 返回和 QQ 自发回声以完全相同的 `message_id` 且双方均唯一为确认关联条件；早到回声仅保留事实，不靠文本或时间猜。超时、缺少 `message_id`、重复回声都不宣布确认。
- 新的中心 O 在原事件中保存供应商原生 assistant 字段及模型来源，包含思考全文、自言自语正文和有序工具调用；R 仍是一个正式事件，投影时可展开成多条原生 tool 消息。完整原生块末尾派生一条 `<event_refs>` user 标签，列 O、各行动的 `O#位置` 与工具名、R；无工具输出只列 O。标签不入日志、不取号，映射归 O，cover 时随原生块一起退出。预算计入标签 token，但事件数仍按 O/R 计算；原生全文超限时尝试有界文本投影，绝不留下孤立 tool 或标签。未完成的行动只显示已有事实，不补造结果。`cover_events` 把 O、其全部 R 和关联的 say 回声一起移出默认上下文，原文仍可反查。清理可见性不销毁日志、不复用号码，长期遗忘和物理退休另待裁定。旧 `condensed`/`clear` 日志只为重放既有生产数据保留只读兼容，不再有写入工具或命令。
...
中心 agent 的 `keep` 将完整原生思考随 O 留到 cover 或历史预算淘汰，并在完整 O/R 时原生重建；`drop` 只将 O/R 作不带思考的已编号文本投影。两种模式都在同一 O 事件中留原文供按号反查。单句 `.chat` 和子代理的独立请求上下文仍使用原生工具配对，`drop` 在其工具循环内把思考字段换成空字符串。
...
完整子响应先登记一个输出号、再派发工具；同步工具完成后登记一个整批结果 R。中心 DeepSeek 会话保留完整的原生 assistant/tool 配对，后续激活按同样的历史预算从 O/R 重建；不完整批次与旧记录用文本投影。下一次请求再追加通知和显式 FIFO pull。流式文本仍由回调逐段产出用于终端显示，不会分拆成多个输出事件；工具失败不会回滚已经发生的行动。单句 `.chat` 不读取 mail，继续通过原生配对获取同步返回。

若供应商返回 `reasoning_content`，流式路径会完整拼接该字段、非流式路径直接读取；中心 O 原样保存到事件文件，未覆盖且仍在预算内时可带回下一次 DeepSeek 请求。QQ 回复和 chatlog 不存这份思考。旧 O 只有存在标记，不能由系统补造思考。
...
- **FIFO 与档案分开。** `mentions(source)` 直接返未读提及及 `message_id`，`pull(source, count)` 在安全边界从指定 FIFO 队首正式读取，`mark_read(source)` 把调用时的水位推到底但不生成 input。`read_messages` 按窗口内 `message_id` 或 `origin` 直接返前后文，不依赖水位；同步返回形成一个 R，不自动分页。
- **召唤不是正文。** 每次通知列出当前水位后的全部未读唤醒；模型输出持久后只确认这次叫醒已经递交，不改变 FIFO 水位。因而同一条召唤在 `pull` 或 `mark_read` 前仍是新的未读，但红点本身不反复启动模型；只有后来新到的召唤或模型已经显式安排的 pull 会让中心 reader 继续，并再次带上当时的完整未读唤醒集合。`final_call` 或取消后的 R 留在普通历史，下次激活按预算重建，普通未读本身也不单独开轮。
