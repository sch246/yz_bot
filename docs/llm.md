...
- 记录**按时间归并**回聊天消息之间，而不是整块堆在前面——工具调用当时就是与聊天交错发生的，堆在前面会让模型以为自己先做完了所有事、然后才有人说话。归并的单位是**一轮**：`assistant(tool_calls)` 和它的 `tool` 结果之间插进聊天消息会拆散配对，所以每轮带一个时间、整体落位。同一时刻聊天排在工具轮之前，因为工具是被那条消息触发的。
- 归并出的顺序**不保证与当时完全一致**：一轮里几个并发调用完成时间不同，工具执行期间到达的消息其真实先后也无法从一个时间点还原。这是明知的近似——拿回"发生过什么、大致在什么时候"，不追求逐条复原。
- 另一处已知不对称：`get_msgs` 会因 token 预算丢掉更早的聊天消息，而工具轮不受那个预算约束，所以可能出现比现存最老的聊天还早的工具轮排在最前面。工具轨道有自己的字符上限兜底。
- `.chat` 单句请求没有可归并的时间轴，退回旧行为：工具记录整块在前。
- cid 是跨轮稳定的 uid，不是每轮重排的序号。原因不是"稳一点更好"：结论住在模型那条 `condense_ops` 调用的 arguments 里，而那条调用会被重建进以后每一轮；若编号每轮重排，模型读到自己写的 `["op3","op4"]` 时这两个号已经指向别的调用，而它无从察觉。能重算的（tool result 的 `[opN]` 前缀）才可以是视图。cid 只需窗口内唯一。
- 上下文因此会随调用增长，**压缩不是自动的**：`condense_ops(cids, conclusion)` 由模型在得出结论时显式调用，把点名的那几轮从轨道和当前上下文里**删掉**。它不在原地补结论——结论已经在这次调用自己的 arguments 里，那条 assistant 消息就在上下文中，再插一条就是第二个副本；工具的返回值也刻意不回显结论。主流 agent 用子代理绕开这件事（子代理跑完只回结论，主上下文从没见过过程），这里是正面解决。
...
- 两个兜底上限按字符算：单条 `MAX_ENTRY_CHARS`、单窗口 `MAX_TOTAL_CHARS`，超了按**整轮**丢最旧的（丢半轮会让重建的配对断裂）。兜底不是压缩机制：按大小自动砍会在结论产出之前砍掉前提。
