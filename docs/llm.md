...
`init_chat()` 最终按以下顺序建立请求消息：当前窗口选择的提示词、内置 base 提示、统一工具模块提示、当前群/私聊位置说明、本窗口的操作历史（有才加，见下）、聊天历史。工具模块提示从开局就列出每个 last-good 模块的名称和第一行描述；模块被 `load_tools` 激活后，其余内容原地加入同一条既有 system 消息，不会在 assistant tool call 与 tool result 之间插入新消息。提示词可以是全局 `settings`、`prompts` 中命名设定，或当前窗口直接保存的消息列表。
...
- 每次工具调用出结果时，`llm.Chat.on_tool_result` 记一条并返回该次调用的 **cid**（`op1`、`op2`……）；cid 会加在发给模型的 tool result 内容开头，模型因此能指名道姓。轨道里存的是没加前缀的原始内容。
- 下一轮 `init_chat` 把整条轨道作为一条 system 消息放进上下文，位置在位置说明之后、聊天历史之前——它是"你做过什么"，不是对话的一部分。
- 轨道不该无限增长，但**压缩不是自动的**。`condense_ops(cids, conclusion)` 由模型在得出结论时显式调用，同时缩短两处：轨道里那几条换成一条结论，当前上下文里那几轮 assistant/tool 消息也被 `Chat.condense_calls` 换成一条结论消息。工具调用往往是为某个目的服务的，结论一旦得出中间过程就只剩噪音；主流 agent 用子代理绕开这件事（子代理跑完只回结论，主上下文从没见过过程），这里是正面解决。
- 收缩按**整轮**进行：一条 assistant 消息里并发的多个 tool_calls 必须一起点名，否则拒绝——协议要求每个 `tool_calls` id 都有对应 tool 消息，只删一半会让请求被拒。正在执行、结果还没回来的那一轮也拒绝收缩。点名的调用如果来自更早的轮次（已不在当前 `messages` 里），当前上下文不动，轨道照常收缩。
- 收缩会让前缀缓存从收缩点之后失效——删消息必然如此。这是自愿付的代价，换来后面每轮都更短；追加式那套"绝不回头改写"的纪律是为了保住缓存，这里是明确地用缓存换长度。
- 单条记录会截断（`MAX_FIELD`），条数有一个远高于正常用量的兜底上限（`MAX_ENTRIES`）。兜底不是压缩机制：按条数自动截断会在结论产出之前砍掉前提，那正是这套设计要避免的。
- `#ops` 查看本窗口轨道，`#ops clear` 清空。清空不会让 cid 计数器归零，否则旧上下文里印着的 cid 会指到不相干的操作上。
