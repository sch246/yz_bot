...
主窗口模型可用 `cover_events(ids, conclusion)` 将本次已读的正式事件号归入当前行动的结论；覆盖节点使用本次输出号加行动位置，不另发编号。一次提交覆盖输出的完整行动批次、结果和唯一确认的 `say` 回声；未确认的自发回声不能先单独覆盖，未见或跨窗口事件不能覆盖。成功后当前会话及下一轮自动投影不再载入成员，`recall_events` 仍能按原号反查，包括总结节点实际冻结的完整成员集合；反查不解除覆盖。覆盖只动事件流的投影与持久索引，不改原始聊天历史或磁盘原文；再次用 `cover_events` 可盖住上一层可见总结，而旧层覆盖不会因此失效。`.chat` 单句及子代理只能保留自己的原生工具收缩，不能提交主窗口覆盖；主窗口正式号的顺序只描述其 mail reader 的读取，不表示所有私有模型首次看到结果的时序。旧 chatlog 中能稳定定位、带 `message_id` 且未撤回的消息与戳一戳，在主模型实际读取时按档案位置补入正式号；v0 或缺消息号的旧消息保守跳过。**迁移限制：**若此前已有缺少档案来源定位的正式输入日志，同一条旧消息可能再次补号；部署前须核实或制定迁移策略，不把这层称为已完成全量兼容。

主窗口 mail reader 还会在每次模型子请求读完 mail 后，按本次 `Chat.messages` 中实际可见的已编号事件重算一次**模型可见的压力 hint**；它不是用户 `#hint` 命令，也不进持久流、聊天历史或 QQ。事件数或文本 token 估算达到对应 `#limit` 的约 75% 时出现，低于两项阈值时消失。它尝试点名最老、当前可覆盖且对受压限额有净收益的一段（最多 12 条），列出连带事件的正式号、条数和估算 token；候选与真实 `cover_events` 共用只读资格和闭包检查。事件数受压时，覆盖超过 2 条即可腾出事件位，即使原文很短；但摘要输出和返回可能反而增加 token。只有 token 受压时，候选原文还须超过暂定的 120 token 摘要预留。若最老段关联未读、隐藏、已收缩、未确认回声等不可覆盖成员，或对受压限额无明显收益，hint 只报告压力和阻碍，不伪称已压缩。75%、12 条和摘要预留 120 token 是暂定启发式，不是安全保证：限额只管历史选取，不等于 API 完整上下文容量；未读 mail 可能一次越线，首次请求可能已过大，也不保证硬上限前至少提醒一轮。模型仍决定是否写摘要，覆盖本身不删除原文。
...
`image`（图片档位）、`reasoning`（工具循环内是否带回思考内容）、`tools`（工具状态的呈现方式）、`max_events`、`max_token` 是同一类东西：**一份窗口级的值 + 一个缺省值**。它们共用一张表（`mods/chat.WINDOW_SETTINGS`），每行是「storage 键 / 默认值 / 归一化」，读取一律走 `chat.window_setting(name)`——窗口里写过的合法值优先，否则默认值。加一项配置就是加一行；归一化负责把存坏的值拉回默认，所以读取端永远拿得到能用的值。
