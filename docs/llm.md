...
`peek` 与 `recall_events` 的每次整条返回各登记为一个新的 result 正式事件。即使返回正文展示多行旧聊天档案，那些行也不分别取得 input 号、不消费 FIFO，且不能充当或确认 `say` 的 QQ 回声；要给未读来源消息分配 input 号须 `pull`，已经有 input 号的旧消息则按该号 `recall_events`。
...
**旧档案主动翻阅。** 中心 agent 用 `peek(source, before)` 按本地 `_log_origin` 锚点预览有界片段，不减少当前 FIFO 未读。远端旧档先用 `fetch(g/u)` 固化成有名字的信源；完成后仍以同一个 `status/peek/pull` 接口查看与读取，不让模型管理 NapCat 请求页。预览返回在实际阅读时取得新的经历号。默认上下文不再为补满预算自动倒扫 chatlog。`#hint` 不触发旧档阅读。
...
- 思考只留“曾有思考”标记，不持久化思考全文；自言自语正文与行动共享一个输出号，并跨轮按统一纯文本投影重建，直到被覆盖或超出上下文预算。工具结果原文完整持久化，自动投影每条有界截取并提示 `recall_events(offset)` 续读，避免单条结果压满请求；覆盖只改默认可见性，不删原文。私有 `.chat` 仍可用 `condense_ops` 收缩自己已经收到的原生工具配对；`recall_ops` 按行动引用取回。清理可见性不销毁日志、不复用号码，长期遗忘和物理退休另待裁定。
...
完整子响应先登记一个输出号、再派发工具；连续聊天在执行中临时暂存 assistant/tool 原生配对，执行完即以编号输出的纯文本投影替换。下一次请求追加插话、显式 FIFO pull 和工具返回的正式号；跨轮重建沿用身份、顺序与自言自语正文。流式文本仍由回调逐段产出用于终端显示，不会分拆成多个输出事件；工具失败不会回滚已经发生的行动。单句 `.chat` 不读取 mail，继续通过原生配对获取同步返回。
...
- **预览与正式拉取分开。** `peek` 读有界最近、旧档或独立信源队首，工具返回成为新的阅读经历但不动未读。`pull(source, count)` 在安全边界从指定 FIFO 队首正式取模型要求的数量；单条超大内容可按档案位置续读。工具结果有独立的有界优先批次。
- **召唤不是正文。** 模型输出持久后确认已递交通知；同一条召唤不因正文仍 pending 而反复唤醒。只有待返回结果、后来新到的召唤或模型已经显式安排的 pull 会让中心 reader 继续；普通未读本身不单独开轮。
- **`#` 只参与控制，不进入模型正文。** 这类事件可留下到达和激活事实（`#poke` 会唤醒），但不会作为聊天正文投影；本地子命令仍就地执行。
- **收尾没有缝。** `context.finish_turn` 在全局 reader 登记锁内同时检查结果、通知与显式 pull；新召唤要么被当前 reader 接住，要么在注销后领取新 reader。
