...
### 末尾 hint 层（设计方向已定，尚未实现）

现状里模块目录是一条 system 消息，由 `SessionBinding._render` 就地改写。这有两个后果：模块目录是上下文里最常变的一段，就地改写会让它之后的全部前缀缓存失效；模型也只有主动调 `list_tools` 才知道磁盘变了。

方向是按缓存原理再分一层，可参考 deepseek-harness 注入 context 的做法：

- 频繁变化、且不需要进历史的内容归为 **hint**，始终追加到上下文末尾，但不写入历史。
- hint 与 system 提示词一样支持用函数生成，只是重置时机不同。
- 工具变化由 hint 主动提示，模型不需要为了发现变化而调用 `list_tools`，从而省掉一个工具。

这是 llm 侧的通用能力，不属于 `mods/tools`；落地时要连带改 `Chat` 的消息组装。同一层还欠 `mods/tools` 的子目录索引：模块内容应能进一步索引子文件夹，而当前 `_source_paths` 只扫描顶层。
