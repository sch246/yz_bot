...
- 没载入不等于丢失。存储侧**唯一**的上限是保留期（`MAX_AGE_SECONDS`，7 天），没有条数上限也没有字数上限：一次巨量输出照存不误。进不进上下文由预算决定，还能不能取回由保留期决定，这是两件事。上下文开头会有一行 `更早还有 N 次工具调用未载入（op1–op12）`，模型用 `recall_ops(cids)` 按 cid 取回完整原文。没有这行指路，模型不知道它们存在、也猜不出 cid，那个入口等于够不着。
