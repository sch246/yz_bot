...
已经确认的目标行为是：cond 异常留下 incident 后沿 fail 继续，让其它反应仍可工作；action 异常停止本节点 action 和成功后继，不把半执行状态当成功。action 可能已经产生部分副作用，因此异常后是否继续普通 `fail` 分支仍需在实现时明确，不能默认当作一次干净的条件失败。
