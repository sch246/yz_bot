...
> 状态：未实现的提案，但**前置实测已完成，不再阻塞**（结果见 §C，草籽 2026-09-19 抓 5701 实测）。本文记录判断与目标形状，不描述当前运行行为。
> 当前事实以[运行架构](../../architecture.md)为准；私聊窗口的身份约定见[私聊窗口的身份](window-identity.md)；关卡现状见 `mods/bot.py` 的 `_route` 与 `ce0e8b2`、`0a334b7` 两笔的提交说明。
...
1. 关卡挪位 + 删掉 `record_sent` 与它的两个调用点 + 终端标签，**同一笔**。`reportSelfMessage` 已经开着，所以这一笔落地即生效，不需要另外改配置。
2. 观察一段：私聊两个方向、群聊、合并转发各自在 chatlog 里只出现一次，且窗口正确。
