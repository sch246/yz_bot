...
> | 2.2 剥离只做了一半 | **仍然存在**，见该节末尾 |
> | 2.3 两个死接口 | **已删除** |
> | 2.4 上下行不对称 | **已解决**——两个方向都是原文 |
> | 2.5 模型侧视图 | **解决一半**——模型现在看得见 reply，at 仍然只有号码 |
> | 5 待定问题 | 第 2 条已回答（link 条件读 `body()`）；第 1 条只回答了一半（去 reply、不去 at，即 §2.2）；其余未定 |
...
**仍然存在。** `msgs.body()` 继承了同一个边界：它只去掉开头的 reply CQ，紧随其后的 at 原样留在正文开头，因此上面列的每一条前缀判定在「引用回复」时照旧失效。这一节是本文剩下的唯一未修缺陷，也是 §5 第 1 条真正要回答的问题：修法是给前缀分派单独一个「连开头的 at 一起去掉」的投影，而不是把 `body()` 本身改激进——`body()` 同时服务 link 条件、延续式回答和单 CQ 判定，那三处去不去 at 的答案并不一样。
...
**已取其前半。** 落地的判据是一句话：**解释**消息的地方读 `msgs.body()`，**存储和显示**消息的地方读原始 `message`；按此逐处定过的消费点是 bot 路由前缀、link 条件与四处输入收集、chat 的 `#` 过滤与唤醒判定、`py.match`、翻页导航、cave 收集与确认、file/dir 的确认与单 CQ 取值、jrrp、cpp、edit。没做的是后半：`segments()` 与模型侧的结构化渲染仍然不存在，`msg_split` 依旧只认 image。
