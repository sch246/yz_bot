...
- [给 agent 做一个聊天软件：主观时间轴，以及它推翻的东西](chat-client-for-an-agent.md)（论点与讨论记录，不是实施提案；2026-09-20 那串讨论走到的地方——柚子是一个主体在用一个聊天软件，而**模型看见的顺序与消息发生的顺序是两条轴**。含来龙去脉、它推翻的三条前提、以及一张「哪些结论有独立证据、哪些只靠这个隐喻站着」的对照表）
...
- [权限演进](permissions.md)（未实现；2026-09-20 追加已定的一条：**Bot 的调用恒以 bot 账号自身权限运行，不因阅读消息而改变**，「代行」是唯一的例外类别）
- [多项目共存](multicore.md)
