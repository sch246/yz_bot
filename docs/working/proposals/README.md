...
- [中心 reader 简化与大文件拆分](reader-cleanup-and-split.md)（代码已实现，尚未部署/重启：固定历史起点、全文同批阅读、oplog 写前校验与故障停批、删除 Mailbox、`chat`/`oplog` 拆包、离线入口隔离；旧 `_stream_results` 恢复桥待生产确认后删除；本篇保留实施前计划和证据）
