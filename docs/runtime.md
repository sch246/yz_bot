...
- 当前工作树的离线补回在监听新事件前冻结已有窗口的 chatlog 锚点与 mail 前缀，启动后由后台线程按 NapCat `message_seq` 翻页；档案原文进按日 `.backfill.jsonl`，信源页与消费位置进 `event_stream`。该窗口补回未收束时 `pull_mail` 立即返回“补回中”，收束后可重试；失败留下可见缺口，不用控制台展示日志补写聊天。独立旧档信源通过 `fetch_source` 创建或扩展，不会倒插已经正式阅读的内容。尚未对真实停机空窗做端到端验收，不应仅凭 smoke 宣称上游历史无缺。
