...
- 重启后的“续话”：`.reboot` 仍记录发起者是否为 Bot 自己；它可在启动完成后触发一次 `chat.chat()`，但现在领取的是同一个中心 reader，不再恢复某窗口独占的主会话。启动后的未确认通知和未读工具结果会从事件流恢复；已确认通知留下的 FIFO 未读不会仅因重启自动唤醒或自动拉取，已登记输出的工具也不会自动重放。人自己执行 `.reboot` 不因该命令单独续话；恢复在 `mods.wait_booted()` 后运行。
- 离线补回在监听新事件前冻结已有窗口的 chatlog 锚点与 mail 前缀，单个窗口准备失败只记日志、不阻止必需的实时监听。启动后后台线程按 NapCat `message_seq` 分批向旧端请求：无可靠本地锚点时首次只接纳最近一批，旧档留给 `fetch(g/u)`；有可靠锚点时持续追到命中锚点、上游确实耗尽或请求链失败，不用请求次数或时间阈值伪造终点，短的非空响应也不是远端尽头。档案原文进按日 `.backfill.jsonl`，内部块索引与消费位置进 `event_stream`。该窗口补回未收束时 `pull(g/u, count)` 立即返回“补回中”，收束后可重试；失败留下可见缺口，不用控制台展示日志补写聊天。独立旧档信源通过 `fetch` 创建或扩展，不会倒插已经正式阅读的内容。尚未对真实停机空窗做端到端验收，不应仅凭 smoke 宣称上游历史无缺。
