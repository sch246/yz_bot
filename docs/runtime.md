...
首次没有 `config.json` 时，Bot 会在终端输出四位验证码，等待某个私聊用户回传，以此设为首位 master；随后继续在私聊中设置 Bot 昵称，并写入默认的 `"bot_permissions": {"op": true}`。旧配置在下次启动时也会补上这项。它单独声明 Bot 自己能否进入宿主级 op 信任域，不要求把 Bot 账号放进人类 `ops` 名单；设为 `false` 后需完整重启才生效。正常接通时终端会依次出现“连接完成”“加载完成”和账号启动信息。这个流程会读取真实 NapCat 事件，不能在生产账号上随意重演。
...
- **派发要挡住。** 派发会让 Bot 自己的话走命令、shell 和 link：`.` 开头当命令跑，`!` 开头过 op 门——而 Bot 作者的 op 结果来自固定的 `config.bot_permissions.op`。于是「检索/网页里的不受信文本 → 模型复述 → 自己执行」会成为一条完整的路。
...
监听器只绑定 `127.0.0.1`，只接受 `POST`，但不校验路径或鉴权；任何能访问本机端口并提交可解析 JSON 的进程都能注入事件。本机边界就是它目前的信任边界。
...
- 重启后的“续话”：`.reboot` 仍记录发起者是否为 Bot 自己；它可在启动完成后触发一次 `chat.chat()`，但现在领取的是同一个中心 reader，不再恢复某窗口独占的主会话。启动后的未确认通知和未读成员由 `oplog` 恢复；已登记的工具结果只在普通已读历史中重建，不独自唤醒。已确认通知留下的未读不会仅因重启自动唤醒或自动拉取，已登记输出的工具也不会自动重放。人自己执行 `.reboot` 不因该命令单独续话；恢复在 `mods.wait_booted()` 后运行。
- 离线补回在监听新事件前冻结已有窗口的 chatlog 锚点与 `oplog` 到达边界，单个窗口准备失败只记日志、不阻止必需的实时监听。启动后后台线程按 NapCat `message_seq` 分批向旧端请求：无可靠本地锚点时首次只接纳最近一批，旧档留给 `fetch(g/u)`；有可靠锚点时持续追到命中锚点、上游确实耗尽或请求链失败，不用请求次数或时间阈值伪造终点，短的非空响应也不是远端尽头。档案原文进按日 `.backfill.jsonl`，内部块索引与消费位置进 `event_stream`。该窗口补回未收束时 `pull(g/u, count)` 立即返回“补回中”，收束后可重试；失败留下可见缺口，不用控制台展示日志补写聊天。独立旧档信源通过 `fetch` 创建或扩展，不会倒插已经正式阅读的内容。尚未对真实停机空窗做端到端验收，不应仅凭 smoke 宣称上游历史无缺。

中心 reader 的这轮简化已在仓库代码实现，但尚未部署到真实 Bot、也未重启验证。部署前不能把 `--check`、临时 `--smoke` 或离线回放当作生产迁移成功：需由维护者决定停机和重启，并在获准检查生产状态后确认旧 `_stream_results` 到达是否已全部处理；兼容桥在确认前保留。
