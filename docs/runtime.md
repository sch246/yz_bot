...
模型工具不把“谁触发了这一轮”当作安全边界：混合上下文里任何人的文字本来都可能影响模型，按触发者切换权限挡不住这条路径，反而会让同一串工作随消息顺序掉权限。这里明确选择 Bot 的固定权限。代行仍是独立路径：`recvmsg(sender_id=X)` 与 `cmds__run_command(sender=X)` 把事件作者写成 X，下游命令按 X 的 `ops` 成员关系判定；外层 Bot 的权限不会提升 X。
...
- 重启后的“续话”：`.reboot` 会记录发起者是不是 Bot 自己（`history.author` 是否为 bot），是的话启动完成后再在该窗口开一轮聊天。人自己 `.reboot` 不会续，因为那是为了换代码，不是有事没做完。续话用的是“设置当前事件 + `chat.chat()`”，不伪造入站消息，所以 `^C`、插话、`#hint` 的行为都和正常聊天一致；它等 `mods.wait_booted()` 之后才开，避免在还没加载完的模块上跑。
