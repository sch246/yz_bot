...
每个来源窗口有跨轮 `context.Mailbox`。路由在同一窗口锁内依次写入 chatlog/history 和 mail，`oplog` 持久记录到达；这保证入列顺序，不表示不同文件有跨文件崩溃原子性。唯一中心 reader 用 `AGENT_WINDOW` 持有全局执行线。激活只产生可确认的通知，不读取正文；通知是创建时快照，列出未读唤醒的时间与作者，不显示 QQ `message_id`，不能拿“通知曾递交”冒充“消息已读”。`status` 查状态，`fetch` 补旧档，`take(start, count)` 在执行时只按当前未读成员的 1-based 序号选范围并于下一 provider 请求前逐条登记正式 input；精确成员 key/arrival/origin/message_id 是高级入口。`mentions` 正式消费筛中的未读提及（至多 500 条），`pull` 是前缀别名但 `read_via` 记录 `pull`。每个正式 input 在写入时保留发起输出号 `read_by` 和工具名 `read_via`；`event_links` 的 output `reads` 从这些 input 事实派生，不从位置猜。`mark_read` 将调用时已有成员跳过，保存发起输出号供 `skipped_by` 投影，不分配 input 号。来源桥随新 input 的持久 projection 归属该 input：此前正式读取的成员沿用首次冻结的投影和原号，跳过成员从原 arrival 或不可变 source 页的档案位置恢复；连续桥超过 5 条只投影首尾与折叠数量。窗口来源顺序由轻量 arrival 索引和页顺序派生，已读/跳过状态只从 append-only input/mark_read 日志派生，Mailbox 修剪和重启不改变语义，source 页格式不变。档案查询 `read_messages` 按指定窗口的 `message_id` 或稳定 `origin` 选前后文，下一请求通过同一消息阅读路径形成新的 archive input；若命中未读 live/source 坐标则一并消费。已读档案可再次阅读，但不冒充实际接收的 say 回声。Python `chat.unread_members(source, limit=...)` 返回脱离权威的普通列表供筛选；大信源应显式限制物化数量。输出在派发工具前持久登记；同一模型输出的全部同步工具返回完成时立即登记一个普通正式 R。当前会话暂存其完整投影，下一子请求先追加这些 R，再处理通知和显式 take；同步 R 不按工具或预算分页。`final_call`、取消、失败或重启后不确认交付、不单独唤醒；R 已在 `oplog`，下次激活按普通历史预算重建，需要全文可 `recall_events`。旧运行版本可能留下 `_stream_results` mail arrival，下一次激活只将它登记或复用为普通 R 后消费，不作为新结果的常规路径。模型输出确认这次“叫醒”已经递交；只有正式 input 或跳过才使 pending 成员退出。红点本身不反复调度模型；后来有新的尚未递交唤醒时才再次启动，并在通知中重列整个未读唤醒集合。旧版首读 floor 仅重放历史日志，新代码不再写入。

中心 agent 的 `cover_events` 在 `oplog` 追加覆盖事实，节点沿用输出号及行动位置；成员可跨来源窗口、允许多重引用，只能来自可见已读或明确反查的旧号，强绑定输出／结果／已确认回声仍成组。覆盖只隐藏默认投影，不删除原文或正式号。`recall_events` 沿已读追加顺序按显式 ID、中心半径或双端点选择，不设条数上限；之后才按种类和来源筛选，并把完整选择作为一个同步 R 直接返回。它不进入未读信源，也不做字符分页。全局 `max_events` 与 `max_token` 共同选唯一已读流的可见后缀，摘要也占名额；默认分别为 500 条和 40000 token，`#limit` 与 `#agent limit` 写入同一份全局设置。`聊天开始`／`聊天结束`是普通内容。旧窗口设置保留但不并入全局设置，独立 `.chat` 仍按窗口请求。旧 chatlog 由档案查询主动翻阅；更早的 NapCat 历史先经 `fetch` 固化为有名字的信源。启动恢复与主动 fetch 内部仍以 NapCat 请求批次抓取，并用不可变持久页保存引用；已读坐标从 input 与标为已读日志派生。页号和上游游标不暴露给模型。`recall_events` 返回在同步工具批次完成时随整批成为一个 R；本地 `_log_origin` 与远端 `message_seq` 只是存储与抓取定位，不是第二套记忆号。sidecar 身份索引、合并阅读索引与启动时的旧信源成员表都只是可重建投影；JSONL、日档与 oplog 仍是各自事实的唯一权威。普通日档没有 `message_seq`，因此不凭 `message_id + 秒级时间` 自动吞并 sidecar；跨来源歧义宁可保留重复记录，也不隐藏可能不同的事实。`#` 控制消息不投影。主模型与辅助调用费用记入 Bot 账，私有 `.chat` 仍记发起者；退休尚未接通。
