...
每个来源窗口有跨轮 `context.Mailbox`。路由在同一窗口锁内依次写入 chatlog/history 和 mail，`oplog` 持久记录到达；这保证入列顺序，不表示不同文件有跨文件崩溃原子性。唯一中心 reader 用 `AGENT_WINDOW` 持有全局执行线。激活只产生可确认的通知，不读取正文；每次通知列出全部当前未读唤醒及其时间、作者与 `message_id`，不能拿“通知曾递交”冒充“消息已读”。`status` 查状态，`fetch` 补旧档，`take` 从有序未读成员中选取并在下一 provider 请求前逐条登记正式 input；`mentions` 是提及筛选薄入口，`pull` 是前缀薄别名。`mark_read` 将调用时已有成员设为已读而不分配经历号。档案查询 `read_messages` 按指定窗口的 `message_id` 或稳定 `origin` 选前后文，下一请求通过同一消息阅读路径形成新的 archive input；若命中未读 live/source 坐标则一并消费。已读档案可再次阅读，但不冒充实际接收的 say 回声。窗口未读的成员洞从已有 input/mark_read journal 重放，source 页保持不可变，已读 `(source,page,offset)` 只从既有 input/source journal 事实派生，不另写 bitmap 或连续消费游标。Python `chat.unread_members(source, limit=...)` 返回脱离权威的普通列表供筛选；大信源应显式限制物化数量。输出在派发工具前持久登记；同一模型输出的全部同步工具返回完成时立即登记一个普通正式 R。当前会话暂存其完整投影，下一子请求先追加这些 R，再处理通知和显式 take；同步 R 不按工具或预算分页。`final_call`、取消、失败或重启后不确认交付、不单独唤醒；R 已在 `oplog`，下次激活按普通历史预算重建，需要全文可 `recall_events`。旧运行版本可能留下 `_stream_results` mail arrival，下一次激活只将它登记或复用为普通 R 后消费，不作为新结果的常规路径。模型输出确认这次“叫醒”已经递交；只有正式 input 或标为已读才使 pending 成员退出。红点本身不反复调度模型；后来有新的尚未递交唤醒时才再次启动，并在通知中重列整个未读唤醒集合。旧版首读 floor 仅重放历史日志，新代码不再写入。
