...
- 失败候选仍留在 `commands`，之后仍能被匹配并在调用时触发 `KeyError`。这是应直接修复的加载器缺陷：导入失败的命令不应继续进入可调用候选表。
- `main.recv()` 在处理任何事件前直接取得 `modules['py']`，所以 `.py` 导入失败会影响所有事件。这反映的是现架构把 `.py`/link 作为主路由核心能力的硬依赖，不能只靠局部修改加载器完成隔离。只有决定缩小 core、允许 `.py` 降级时，才需要随相应架构切分解除这条依赖。
...
这延续了“普通函数可被多处组合”的优点，也形成一条独立能力边界：当前工具中包含 `.py` 共享环境执行和用户 storage 修改，而注册过程不按发言者 op 身份过滤。维护者当前以手动控制可信群白名单作为主要运维边界，是否再对触发者做 op 检查仍是可选加固；完整调用循环、当前工具表和边界见 [LLM 文档](llm.md)。
...
当前 `loc` 在执行 `pyload.py` 前建立，link 分派时才刷新；冷启动后的首条直接 `.py` 可能看不到 `pyload.py` 新名称，经过一次会刷新环境的消息后恢复。维护者接受这项短暂启动差异，当前不把它列为待修缺陷。若未来需要收束多个名字来源，仍应保留即时持久化体验，方向见[未实现提案](working/proposals/core-and-failure-domains.md#动态执行环境的收束方向)。
...
绝大多数后来增加的配置和业务状态由 `s3.storage` 持有：命名空间对应目录，键对应 JSON 文件，模块取得普通内存可变对象。Bot 运行期间内存是写入权威，JSON 是可人工编辑和修复的持久投影。每项保存上次内存与文件一致时的规范 JSON 摘要作为 baseline；纯缩进或键顺序变化不算内容变化。

专用同步线程把所有已载入对象的序列化检查稳定分散在十分钟窗口内，每个 tick 最多执行一次普通内存同步。只有相对 baseline 变化或文件需要重建的对象才原子写盘，因此不会每十分钟集中全量读写。显式 `storage.save()` 和正常退出仍强制保存全部当前对象。写入使用同目录临时文件、`fsync()` 和 `os.replace()`；失败不推进 baseline，并同时写应用日志和终端。

安装 `watchdog` 时，JSON 文件事件会立即进入同步线程；没有该可选依赖时降级为约两秒一轮、按路径分散的 `stat` 检查。低频目录发现同时处理新 JSON 并弥补丢失事件。事件先经过约 1 秒的稳定窗口，避免把编辑器正常的“截断再写入”过程误判为空文件恢复。合法文件变化只有在内存仍等于 baseline 时才原地进入内存；否则打印“文件覆盖内存失败”并保留内存，等待下一次内存同步覆盖文件。这个 CAS 对直接修改的裸 `dict/list` 只作同步前二次摘要检查，是已接受的尽力保证，不是跨线程事务。

文件控制信号有意不遵循普通 JSON 载入：

- 空文件表示立即用当前内存恢复该文件；冷启动尚无对应内存时只能报错；
- 裸文本 `DELETE` 表示业务删除，同时删除内存项和文件；代码也可显式调用 `storage.delete(namespace, name)`；
- 普通文件删除被视为可能的手滑操作，只标记文件待重建，不删除内存；
- 其它非法 JSON 只报告错误，不替换内存。

`storage.load(namespace, name)` 没有被 watcher 取代：它保留为管理员显式选择“文件覆盖内存”的强制入口，并在成功后重建 baseline。启动加载失败会记录 `load_errors`；功能随后 `get()` 认领该项时拒绝用空默认值覆盖坏文件。dict/list 根对象继续原地替换以保留模块已有引用，但嵌套引用和从 storage 派生出的 client、标量 cache 不承诺自动刷新。

选择文件而非数据库最初来自可理解性：JSON、文本和目录可以直接阅读、备份、编辑和修复；不需要数据库服务或迁移器。代价是缺少事务、并发写协调、局部更新和高效查询，但当前规模并不自动要求换数据库。

Config 只有在 ops、昵称和首次启动流程都迁移到 storage 或其它单一权威、且所有读写调用被删除后才能退役；在此之前不能同时维护两份可写管理员真相。尚未实现的逐功能值校验和历史恢复见[Storage 与历史提案](working/proposals/storage-and-history.md)。
