...
每个来源窗口有跨轮 `context.Mailbox`。路由在同一窗口锁内依次写入 chatlog/history 和 mail，`oplog` 持久记录到达；这保证入列顺序，不表示不同文件有跨文件崩溃原子性。唯一中心 reader 用 `AGENT_WINDOW` 持有全局执行线。激活只产生可确认的通知，不读取正文；`peek` 有界预览不减少未读，`pull_mail` 按选定窗口 FIFO 有界正式读取并分配统一号。工具结果走独立的全局待读队列，优先于聊天积压；输出在派发工具前持久登记。模型输出确认通知，页读取提交 pending，二者不是同一水位。首次没有可信旧边界的窗口持久冻结最近一页作为日常追读起点。

中心 agent 的 `cover_events` 在 `oplog` 追加覆盖事实，节点沿用输出号及行动位置；成员可跨来源窗口、允许多重引用，只能来自可见已读或明确反查的旧号，强绑定输出／结果／已确认回声仍成组。覆盖只隐藏默认投影，不删除原文或正式号。`event_span` 沿已读追加顺序按中心半径或双端点选择至多 40 条，之后才按种类和来源筛选；它只列正式号，不替模型读取或覆盖。全局 `max_events` 与 `max_token` 共同选唯一已读流的可见后缀，摘要也占名额；`聊天开始`／`聊天结束`是普通内容。旧窗口设置保留但不并入全局设置，独立 `.chat` 仍按窗口请求。旧 chatlog 通过有界 `peek` 主动翻阅；`peek_napcat` 可只读查询上游当前提供的有界群聊／私聊历史，不消本地未读。启动时以旧 chatlog 锚点逐页补回 NapCat 历史到独立按日 sidecar；有可靠锚点就持续翻到锚点、上游尽头或真实分页失败，没有可靠锚点则首次只接纳最近一页。持久页索引与消费位置由 oplog 记录；原窗口 mail 读完旧前缀后先读补回页，再读开机后实时尾部。主动 `fetch_source` 可为更早历史建立独立 FIFO；`source_status`、`peek_source`、`pull_source` 分别查看状态、预览和正式读取。回放返回在实际阅读时成为新的结果事件；本地 `_log_origin` 与远端 `message_seq` 是各自的排他分页游标，不是第二套记忆号。sidecar 身份索引、合并阅读索引与启动时的旧 FIFO 成员表都只是可重建投影：前两者分别服务稳定 origin／点查和跨文件有界分页，临时成员表只判断原文是否已进过旧 boot FIFO；JSONL、日档与 oplog 仍是各自事实的唯一权威。普通日档没有 `message_seq`，因此不凭 `message_id + 秒级时间` 自动吞并 sidecar；跨来源歧义宁可保留重复记录，也不隐藏可能不同的事实。`#` 控制消息不投影。主模型与辅助调用费用记入 Bot 账，私有 `.chat` 仍记发起者；退休尚未接通。
