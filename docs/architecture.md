...
`history` 因此是纯内存，不再自己持久化：启动时由 `chatlog.on_load` 从文件重建近期窗口，且只重建切换点之后的 v1 消息记录——v0 正文是投影、v0 私聊没有发送者、notice 重建不成事件，放进内存都是静默的错误内容。重建不出东西时内存就是空的，随新消息长回来。
...
终端打印是实际运维界面的一部分：收发消息、流式 LLM 内容、工具调用、图片捕获/缓存、link/capture 命中、模块失败和生命周期进度都应保留可观察输出。这些产出者各自选一条**流名**（`mods.log.stream(...)`，即 `yz.*` logger），终端订阅其中一组；流是「属于哪个子系统」这条轴，与 logger 级别表达的「是不是故障」互不替代。应用 logger 记录诊断；chatlog 保存产品聊天历史，三者不是同一用途。终端行为、订阅集与 `.log` 见[运行时](runtime.md)。
