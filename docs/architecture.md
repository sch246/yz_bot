...
每个来源窗口有跨轮 `context.Mailbox`。路由在同一窗口锁内依次写入 chatlog/history 和 mail，`oplog` 持久记录到达；这保证入列顺序，不表示不同文件有跨文件崩溃原子性。唯一中心 reader 用 `AGENT_WINDOW` 持有全局执行线。激活只产生可确认的通知，不读取正文；每次通知冻结并列出当前水位后的全部未读唤醒及其时间、作者与 `message_id`，不能拿“通知曾递交”冒充“消息已读”。`status` 查状态，`mentions` 直接返尚未消费的提及，`fetch` 补旧档，`pull` 从 FIFO 队首正式读取并分配统一号，`mark_read` 把调用时已存在的队列水位推到底而不分配经历号。档案查询 `read_messages` 独立于 mail，按指定窗口的 `message_id` 或稳定 `origin` 直接返前后文，不改未读水位。输出在派发工具前持久登记；同一模型输出的全部同步工具返回完成时立即登记一个普通正式 R。当前会话暂存其完整投影，下一子请求先追加这些 R，再处理通知和显式 pull；不按工具或预算分页。`final_call`、取消、失败或重启后不确认交付、不单独唤醒；R 已在 `oplog`，下次激活按普通历史预算重建，需要全文可 `recall_events`。旧运行版本可能留下 `_stream_results` mail arrival，下一次激活只将它登记或复用为普通 R 后消费，不作为新结果的常规路径。模型输出确认这次“叫醒”已经递交；只有 FIFO 拉取或标为已读才提交 pending 水位，且只有 pull 生成正式 input。水位后的激活条目仍让红点保持点亮，但红点本身不反复调度模型；后来有新的尚未递交唤醒时才再次启动，并在通知中重列整个未读唤醒集合。首次没有可信旧边界的窗口持久冻结最近的有界尾部作为日常追读起点。

中心 agent 的 `cover_events` 在 `oplog` 追加覆盖事实，节点沿用输出号及行动位置；成员可跨来源窗口、允许多重引用，只能来自可见已读或明确反查的旧号，强绑定输出／结果／已确认回声仍成组。覆盖只隐藏默认投影，不删除原文或正式号。`recall_events` 沿已读追加顺序按显式 ID、中心半径或双端点选择，不设条数上限；之后才按种类和来源筛选，并把完整选择作为一个同步 R 直接返回。它不进入 FIFO，也不做字符分页。全局 `max_events` 与 `max_token` 共同选唯一已读流的可见后缀，摘要也占名额；默认分别为 500 条和 40000 token，`#limit` 与 `#agent limit` 写入同一份全局设置。`聊天开始`／`聊天结束`是普通内容。旧窗口设置保留但不并入全局设置，独立 `.chat` 仍按窗口请求。旧 chatlog 由档案查询主动翻阅；更早的 NapCat 历史先经 `fetch` 固化为有名字的 FIFO。启动恢复与主动 fetch 内部仍以 NapCat 请求批次抓取，并用持久块索引保存引用与消费位置；这些块、页号和上游游标不暴露给模型。回放返回在同步工具批次完成时随整批成为一个 R；本地 `_log_origin` 与远端 `message_seq` 只是存储与抓取定位，不是第二套记忆号。sidecar 身份索引、合并阅读索引与启动时的旧 FIFO 成员表都只是可重建投影；JSONL、日档与 oplog 仍是各自事实的唯一权威。普通日档没有 `message_seq`，因此不凭 `message_id + 秒级时间` 自动吞并 sidecar；跨来源歧义宁可保留重复记录，也不隐藏可能不同的事实。`#` 控制消息不投影。主模型与辅助调用费用记入 Bot 账，私有 `.chat` 仍记发起者；退休尚未接通。
