...
每个来源窗口有跨轮 `context.Mailbox`。路由在同一窗口锁内依次写入 chatlog/history 和 mail，`oplog` 持久记录到达；这保证入列顺序，不表示不同文件有跨文件崩溃原子性。唯一中心 reader 用 `AGENT_WINDOW` 持有全局执行线。激活只产生可确认的通知，不读取正文；`status` 查状态，`mentions` 直接返尚未消费的提及，`fetch` 补旧档，`pull` 从 FIFO 队首正式读取并分配统一号，`mark_read` 把调用时已存在的队列水位推到底而不分配经历号。档案查询 `read_messages` 独立于 mail，按指定窗口的 `message_id` 或稳定 `origin` 直接返前后文，不改未读水位。输出在派发工具前持久登记；同一模型输出的全部同步工具返回完成时立即登记一个普通正式 R。当前会话暂存其完整投影，下一子请求先追加这些 R，再处理通知和显式 pull；不按工具或预算分页。`final_call`、取消、失败或重启后不确认交付、不单独唤醒；R 已在 `oplog`，下次激活按普通历史预算重建，需要全文可 `recall_events`。旧运行版本可能留下 `_stream_results` mail arrival，下一次激活只将它登记或复用为普通 R 后消费，不作为新结果的常规路径。模型输出确认通知，FIFO 拉取和标为已读都提交 pending 水位，但只有 pull 生成正式 input。首次没有可信旧边界的窗口持久冻结最近的有界尾部作为日常追读起点。未完成 FIFO 本身不唤醒或续跑中心 reader；只有新召唤或模型显式安排的 pull 会继续。
