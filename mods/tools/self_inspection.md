...
迭代器逐条读磁盘，不改变未读成员、覆盖或运行中的上下文。Python 扫描过一条记录不等于你在聊天中重新经历了它；只有最终返回给你的筛选结果会成为本次行动返回。

若要把检查结果写成长期记忆，先让结果回到下一次模型请求，再用实际选中的正式号调用 `cover_events`。不要只保存变量名、查询代码或“当时符合条件的集合”，因为以后重跑可能得到不同成员。

聊天原话可按窗口用 `read_messages` 选取，选中的消息在下一次模型请求中成为正式 input，命中当前未读成员时一并消费。有序未读成员可用 `status` 查看、用 `take` 任意选取；`mentions` 是只选未读提及的薄入口，`mark_read` 只设为已读而不展示正文。需要用 Python 自行过滤未读消息时，可从 `ctx["chat"].unread_members(source, limit=...)` 取得脱离内部权威的普通 dict 列表，再把稳定 `key` 列表提交给 `take(ids=...)`；不设 `limit` 会完整物化大信源。这个 Skill 只负责自由检查已经发生的经历，不替代改变事实的提交入口。
