...
WHY: 「全量回写」这一条 2026-09-20 由草籽拍了方向：**压缩记录应当是文件，而且是分散的
文件**（「编辑一个巨大的文本文件是难以令人接受的」），或者特定格式的数据库存储。所以扛不住
时的正解不是这里说的"先加回收"，而是换存储形态——`storage` 只有 `_write_snapshot` 全量
替换、没有追加，而 `chatlog` 的 `<kind>/<window>/<YYYY-MM>/<DD>.log` 分片追加是现成的正确
形状。换过去要连带改的是 `condense`：就地改标记在追加式文件里做不到，得改成追加一条"某某被
某节点收缩了"再在读取时折叠——`chatlog` 已经是这个路子（它重建出来的记录仍然包含后来被撤回
的消息）。按什么分片还没有答案：chatlog 按时间分是因为查询是"最近 N 条"，而树的查询是
"高度 ≤ H"和"盖住 message_id X"，按时间分对两者都不友好。见
docs/working/proposals/mail-and-activation.md 第九节。
...
def _referenced_cids(entry: dict) -> list[str]:
    ...
    不是"等接管者复用的零件"，**它就是接管者本身**：按高度退休与旧 `sweep` 是同一套可达性
    回收，只换了根——根从"窗口内的轮（按聊天时间）"变成"高度 ≤ H 的节点（按深度）"，
    这个遍历一个字不用改。草籽 2026-09-20 定：删除的优先级必然低于保存的优先级，因为层级
    提升靠反查，直接删整棵子树必然误伤还有人引用的东西。所以删除的判据是"高度超限**且**
    无人引用"，而下面这两条约束与门槛用时间还是深度无关，照样成立：
